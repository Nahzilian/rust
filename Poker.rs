...
/* 
   Sort function 
   It will sort by ranks, and then sort by suits if they are the same
*/
fn sort_cards(arr:&mut Vec<u32>){
    let mut swapped = true; 
    while swapped {
        swapped = false; //If there is no change, array is sorted.
        for i in 1..arr.len() {
...
fn has_straight(hand: &Vec<u32>) -> bool{    
    let mut temp = hand[0];
    if hand[4]%13==1 && hand[0]%13 == 2 { //Check when it's the special low Ace case
        for i in 1..4{
...
/* Function to compare sets in tie breaking (assuming they have the same rank)
   Take the output of the function check_match as input
...
fn winner(hand1:&mut Vec<u32>,hand2:&mut Vec<u32>) -> u32{
    ...
    if rank1>rank2 {         //Hand 1 has higher ranking
        ...
        if rank1 == 10 { //Royal Flush - Compare the highest cards
            ...
            if res !=3 {return res} //If not equal return result
            res = compare_by_suit(high1,high2); //If all are equal, compare by suit of the highest cards
            return res;
            ...
            let mut res; //Variable to store results
            res = compare_by_rank(pair1,pair2); //Compare by rank the pairs
            if res != 3 {return res}
            for i in (0..3).rev(){ //Compare the ranks starting from highest card to lowest card of each hand
...
/* Deal the cards into 2 hands and determine the winner */
fn deal(arr:[u32;10]) ->  [String;5]{
    /* Create scalable arrays for hand 1 and hand 2*/ 
...
/*
    Converting each card value into a string and return an array of String
*/
fn convert_to_string(hand: &Vec<u32>) -> [String;5]{
    ...
    for item in arr.iter_mut(){
        let mut val = hand[count] % 13;
        if val == 0{
            val = 13;
        }
        if get_suit(hand[count]) == 1{
            *item = format!("{}{}",val,club);
        }else if get_suit(hand[count]) == 2{
            *item = format!("{}{}",val,diamond);
        }else if get_suit(hand[count]) == 3{
            *item = format!("{}{}",val,heart);
        }else if get_suit(hand[count]) == 4{
            *item = format!("{}{}",val,spade);
        }
        count+=1;
    }
    return arr;
